...
class VideoOptimizerApp:
    def __init__(self, master):
        ...
        self.master.geometry("520x950")  # 增高窗口
        ...
        # --- 视频信息显示区域 ---
        info_frame = tk.LabelFrame(master, text="原视频信息", padx=10, pady=10)
        info_frame.pack(padx=10, pady=5, fill="x")

        self.info_text = tk.Text(info_frame, height=8, wrap="word")
        self.info_text.pack(fill="x")
        self.info_text.config(state="disabled")  # 初始只读

        # 复制参数按钮
        copy_btn = tk.Button(info_frame, text="复制参数", command=self.copy_info)
        copy_btn.pack(anchor="ne", pady=2)
        ...
        # 视频码率
        tk.Label(master, text="视频码率 (kbps，例如 1500):").pack(pady=5, anchor="w", padx=10)
        self.video_bitrate_entry = tk.Entry(master)
        self.video_bitrate_entry.pack(pady=2, padx=10, fill="x")

        # 音频码率
        tk.Label(master, text="音频码率 (kbps，例如 128):").pack(pady=5, anchor="w", padx=10)
        self.audio_bitrate_entry = tk.Entry(master)
        self.audio_bitrate_entry.pack(pady=2, padx=10, fill="x")

        # 分辨率
        tk.Label(master, text="分辨率 (例如 1280x720，不改请留空):").pack(pady=5, anchor="w", padx=10)
        ...
        # 帧率
        tk.Label(master, text="帧率 (fps，例如 30):").pack(pady=5, anchor="w", padx=10)
        self.framerate_entry = tk.Entry(master)
        self.framerate_entry.pack(pady=2, padx=10, fill="x")

        # 视频编码器选择
        tk.Label(master, text="视频编码器:").pack(pady=5, anchor="w", padx=10)
        ...
        video_codec_menu = ttk.Combobox(master, textvariable=self.video_codec_var,
                                        values=list(self.video_codec_options.keys()), state="readonly")
        video_codec_menu.pack(pady=2, padx=10, fill="x")

        # 音频编码器选择
        tk.Label(master, text="音频编码器:").pack(pady=5, anchor="w", padx=10)
        ...
        audio_codec_menu = ttk.Combobox(master, textvariable=self.audio_codec_var,
                                        values=list(self.audio_codec_options.keys()), state="readonly")
        audio_codec_menu.pack(pady=2, padx=10, fill="x")
        ...
        self.progress = ttk.Progressbar(master, length=480, style="blue.Horizontal.TProgressbar", mode="determinate")
        self.progress.pack(pady=5, padx=10)

        # 取消按钮
        self.cancel_button = tk.Button(master, text="取消转换", command=self.cancel_conversion,
                                       state="disabled", bg="#ff6666")
    ...
    def update_video_info(self, filepath):
        ...
        for track in media_info.tracks:
            if track.track_type == "Video":
                ...
                resolution = f"{track.width}x{track.height}" if track.width and track.height else "未知"
                fps = track.frame_rate if track.frame_rate else "未知"
                video_bitrate = int(track.bit_rate) / 1000 if track.bit_rate else "未知"
            elif track.track_type == "Audio":
                audio_codec = track.format
                audio_bitrate = int(track.bit_rate) / 1000 if track.bit_rate else "未知"
            elif track.track_type == "General":
                duration = float(track.duration) / 1000 if track.duration else 0

        info_text = f"文件路径: {filepath}\n视频码率: {video_bitrate} kbps\n音频码率: {audio_bitrate} kbps\n" \
                    f"视频时长: {duration:.2f} 秒\n分辨率: {resolution}\n帧率: {fps} FPS\n" \
                    f"视频编码格式: {video_codec}\n音频编码格式: {audio_codec}"
    ...
    def copy_info(self):
        ...
        self.master.clipboard_append(self.info_text.get("1.0", tk.END))
        messagebox.showinfo("复制成功", "视频参数已复制到剪贴板！")
    ...
    def start_conversion(self):
        ...
        output_path = os.path.join(
            self.output_var.get(),
            os.path.splitext(os.path.basename(self.file_var.get()))[0] + "_optimized" +
            os.path.splitext(self.file_var.get())[1]
        )
